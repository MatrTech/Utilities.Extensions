...
namespace MatrTech.Utilities.Extensions.Common.UnitTests
{
	...
	public class StringRandomTests
	{
		...
		public void Random_NoExceptInput_ReturnCorrectLength()
        {
			...
			generated.Length.Should().Be(length);
		}
		...
		public void Random_WithExceptInput_ReturnsOnlyA()
		{
            ...
            var expected = Enumerable.Range(0, length).Select(s => "a");
            var generated = string.Empty.Random(length, except);

            generated.ToLowerInvariant().ToArray().Should().AllBeEquivalentTo('a');
            generated.ToLowerInvariant().ToList().ForEach(c => c.Should().Be('a'));
        }
